...
args.gamma_restoration = config.getfloat('DEFAULT', 'gamma_restoration', fallback=0.0)
...
args.monitor_cosine_const = config.getboolean('DEFAULT', 'monitor_cosine_const', fallback=False)
args.relu_g_const = config.getboolean('DEFAULT', 'relu_g_const', fallback=True)
args.actor_input_mode = config.get('DEFAULT', 'actor_input_mode', fallback='concat')
